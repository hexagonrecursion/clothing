...
local function get_stand_object(pos)
  ...
  for _, obj in pairs(objects) do
    ...
    if ent then
      if ent.name == "hades_clothing:mannequin_entity" then
        -- Remove duplicates
        if object then
          obj:remove()
        else
...
local function update_entity(pos)
  ...
  if object then
    if not string.find(node.name, "hades_clothing:mannequin_") then
      object:remove()
      return
    end
  else
    ...
    object = minetest.add_entity(opos, "hades_clothing:mannequin_entity")
  end
  if object then
...
local function add_hidden_node(pos, player)
  ...
  if node.name == "air" and not minetest.is_protected(pos, name) then
    minetest.set_node(p, {name="hades_clothing:mannequin_stand_top"})
  end
end
...
local function remove_hidden_node(pos)
  local p = {x=pos.x, y=pos.y + 1, z=pos.z}
  local node = minetest.get_node(p)
  if node.name == "hades_clothing:mannequin_stand_top" then
    minetest.remove_node(p)
  end
end

minetest.register_node("hades_clothing:mannequin_stand_top", {
...
minetest.register_node("hades_clothing:mannequin_stand", {
  ...
  after_place_node = function(pos, placer)
    local opos = {x=pos.x, z=pos.z}
    opos.y = pos.y + y_change
    minetest.add_entity(opos, "hades_clothing:mannequin_entity")
...
minetest.register_entity("hades_clothing:mannequin_entity", {
  physical = true,
  visual = "mesh",
  ...
  on_blast = function(self, damage)
    ...
    if node.name == "hades_clothing:mannequin_stand" then
      drop_armor(self.pos)
      self.object:remove()
    end
...
minetest.register_abm({
  nodenames = {"hades_clothing:mannequin_stand"},
  interval = 15,
  chance = 1,
...
if minetest.get_modpath("hades_extramaterials") then
  minetest.register_craft({
    output = "hades_clothing:mannequin_stand",
    recipe = {
      {"dye:orange", "hades_materials:plastic_sheeting", "dye:white"},
      {"hades_extramaterials:plastic_strip", "hades_materials:plastic_sheeting", "hades_extramaterials:plastic_strip"},
      {"hades_extramaterials:plastic_strip", "hades_materials:plastic_sheeting", "hades_extramaterials:plastic_strip"},
    }
  })
end
